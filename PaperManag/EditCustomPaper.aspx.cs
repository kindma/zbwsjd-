...
//				else
//				{
					LBEdit.Attributes.Add("onclick", "javascript:var str=window.showModalDialog('EditCustomPolicy.aspx?PaperID="+intPaperID+"&SubjectID="+DataGridPolicy.Items[i].Cells[6].Text+"&LoreID="+DataGridPolicy.Items[i].Cells[7].Text+"&TestTypeID="+DataGridPolicy.Items[i].Cells[8].Text+"','','dialogHeight:497px;dialogWidth:660px;edge:Raised;center:Yes;help:Yes;resizable:No;scroll:No;status:No;');");
...
//			}
			...
			else
			{
				...
				try
				{
					...
					this.RegisterStartupScript("newWindow","<script language='javascript'>alert('修改手工组卷成功！');try{ window.opener.RefreshForm() }catch(e){};window.close();</script>");
				}
				catch
				{
					ObjTran.Rollback();
					this.RegisterStartupScript("newWindow","<script language='javascript'>alert('修改手工组卷失败！')</script>");
				}
				finally
				{
					SqlConn.Close();
