...
{
	/// <summary>
	/// SelectJoinExam 的摘要说明。
	/// </summary>
	public partial class SelectJoinExam : System.Web.UI.Page
	{
		...
		private void ShowSelectedData()
		{
			...
			objCmd=new SqlDataAdapter("select b.DeptID,b.DeptName from PaperUser a,DeptInfo b where a.DeptID=b.DeptID and a.PaperID="+intPaperID+" and a.UserType=0 order by b.DeptName asc",objConn);
			objDS=new DataSet();
			objCmd.Fill(objDS,"DeptInfo");
			for(int i=0;i<objDS.Tables["DeptInfo"].Rows.Count;i++)
			{
			...
			objCmd=new SqlDataAdapter("select b.UserID,b.LoginID from PaperUser a,UserInfo b where a.UserID=b.UserID and a.PaperID="+intPaperID+" and a.UserType=0 order by b.LoginID asc",objConn);
			objDS=new DataSet();
			objCmd.Fill(objDS,"UserInfo");
			for(int i=0;i<objDS.Tables["UserInfo"].Rows.Count;i++)
			{
		...
		protected void ButInput_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				ObjCmd.CommandText="delete from PaperUser where PaperID="+intPaperID+" and UserType=0";
				...
				for(i=0;i<LBSelected.Items.Count;i++)
				{
					...
					{
						ObjCmd.CommandText="insert into PaperUser(PaperID,UserID,DeptID,UserType) values("+intPaperID+",0,"+LBSelected.Items[i].Value.Replace("#","")+",0)";
						ObjCmd.ExecuteNonQuery();
					}
					else
					{
						ObjCmd.CommandText="insert into PaperUser(PaperID,UserID,DeptID,UserType) values("+intPaperID+","+LBSelected.Items[i].Value+",0,0)";
						ObjCmd.ExecuteNonQuery();
					}
				}
				ObjTran.Commit();
			...
			for(i=0;i<LBSelected.Items.Count;i++)
			{
				if (strJoinExam=="")
				{
					strJoinExam=strJoinExam+LBSelected.Items[i].Text;
				}
				else
				{
					strJoinExam=strJoinExam+";"+LBSelected.Items[i].Text;
			...
			this.RegisterStartupScript("newWindow","<script language='javascript'>window.returnValue='"+strJoinExam+"';window.close();</script>");
		}
		#endregion
