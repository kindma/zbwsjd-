...
{
	/// <summary>
	/// SelectJoinManager 的摘要说明。
	/// </summary>
	public partial class SelectJoinManager : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (DDLDeptName.SelectedItem.Value=="0")
			{
				...
				strSql2="select a.UserID,a.LoginID from UserInfo a where a.UserType=1";
				if (txtQuery.Text.Trim()!="")
				{
					strSql2=strSql2+" where (a.LoginID like '%"+txtQuery.Text.Trim()+"%' or a.UserName like '%"+txtQuery.Text.Trim()+"%') and a.UserType=1";
				}
			}
			//显示以部门为条件的数据
			if(DDLDeptName.SelectedItem.Value!="0")
			{
				...
				strSql2="select a.UserID,a.LoginID from UserInfo a,DeptInfo b where a.DeptID=b.DeptID and b.DeptID='"+DDLDeptName.SelectedItem.Value+"' and a.UserType=1";
				if (txtQuery.Text.Trim()!="")
				{
					strSql2=strSql2+" and (a.LoginID like '%"+txtQuery.Text.Trim()+"%' or a.UserName like '%"+txtQuery.Text.Trim()+"%') and a.UserType=1";
				}
			}
		...
		private void ShowSelectedData()
		{
			...
			objCmd=new SqlDataAdapter("select b.DeptID,b.DeptName from PaperUser a,DeptInfo b where a.DeptID=b.DeptID and a.PaperID="+intPaperID+" and a.UserType=1 order by b.DeptName asc",objConn);
			objDS=new DataSet();
			objCmd.Fill(objDS,"DeptInfo");
			for(int i=0;i<objDS.Tables["DeptInfo"].Rows.Count;i++)
			{
			...
			objCmd=new SqlDataAdapter("select b.UserID,b.LoginID from PaperUser a,UserInfo b where a.UserID=b.UserID and a.PaperID="+intPaperID+" and a.UserType=1 order by b.LoginID asc",objConn);
			objDS=new DataSet();
			objCmd.Fill(objDS,"UserInfo");
			for(int i=0;i<objDS.Tables["UserInfo"].Rows.Count;i++)
			{
		...
		protected void ButInput_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				ObjCmd.CommandText="delete from PaperUser where PaperID="+intPaperID+" and UserType=1";
				...
				for(i=0;i<LBSelected.Items.Count;i++)
				{
					...
					{
						ObjCmd.CommandText="insert into PaperUser(PaperID,UserID,DeptID,UserType) values("+intPaperID+",0,"+LBSelected.Items[i].Value.Replace("#","")+",1)";
						ObjCmd.ExecuteNonQuery();
					}
					else
					{
						ObjCmd.CommandText="insert into PaperUser(PaperID,UserID,DeptID,UserType) values("+intPaperID+","+LBSelected.Items[i].Value+",0,1)";
						ObjCmd.ExecuteNonQuery();
					}
				}
				ObjTran.Commit();
			...
			//传给父窗口
			string strJoinManager="";
			for(i=0;i<LBSelected.Items.Count;i++)
			{
				if (strJoinManager=="")
				{
					strJoinManager=strJoinManager+LBSelected.Items[i].Text;
				}
				else
				{
					strJoinManager=strJoinManager+";"+LBSelected.Items[i].Text;
			...
			this.RegisterStartupScript("newWindow","<script language='javascript'>window.returnValue='"+strJoinManager+"';window.close();</script>");
		}
		#endregion
