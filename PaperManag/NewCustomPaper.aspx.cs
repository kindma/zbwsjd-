...
{
	/// <summary>
	/// NewCustomPaper 的摘要说明。
	/// </summary>
	public partial class NewCustomPaper : System.Web.UI.Page
	{
...
//					{
						...
						ButAddPolicy.Attributes.Add("onclick", "javascript:var str=window.showModalDialog('AddCustomPolicy.aspx?PaperID="+intPaperID+"','','dialogHeight:550px;dialogWidth:788px;edge:Raised;center:Yes;help:Yes;resizable:No;scroll:No;status:No;');");
...
//					}
					if (Request["PaperType"]=="1")
					{
						DDLPaperType.Items.FindByValue("1").Selected=true;
					}
					else
					{
						DDLPaperType.Items.FindByValue("2").Selected=true;
					}
		...
		private void ShowPaperPolicy()
		{
			...
			{
				SqlCmdTmp=new SqlDataAdapter("select a.PaperTestID,a.RubricID,b.SubjectID,b.LoreID,b.TestTypeID from PaperTest a,RubricInfo b where a.RubricID=b.RubricID and b.SubjectID="+SqlDS.Tables["PaperPolicy"].Rows[i]["SubjectID"]+"and b.LoreID="+SqlDS.Tables["PaperPolicy"].Rows[i]["LoreID"]+" and b.TestTypeID="+SqlDS.Tables["PaperPolicy"].Rows[i]["TestTypeID"]+" and a.PaperID="+intPaperID+" order by a.PaperTestID asc",SqlConn);
				...
				strSelectTest.Text="";
				for(int j=0;j<SqlDSTmp.Tables["PaperTest"].Rows.Count;j++)
				{
					if (strSelectTest.Text=="")
					{
						strSelectTest.Text=strSelectTest.Text+SqlDSTmp.Tables["PaperTest"].Rows[j]["RubricID"].ToString();
					}
					else
					{
						strSelectTest.Text=strSelectTest.Text+";"+SqlDSTmp.Tables["PaperTest"].Rows[j]["RubricID"].ToString();
				...
				LBEdit.Attributes.Add("onclick", "javascript:var str=window.showModalDialog('EditCustomPolicy.aspx?PaperID="+intPaperID+"&SubjectID="+DataGridPolicy.Items[i].Cells[6].Text+"&LoreID="+DataGridPolicy.Items[i].Cells[7].Text+"&TestTypeID="+DataGridPolicy.Items[i].Cells[8].Text+"','','dialogHeight:550px;dialogWidth:788px;edge:Raised;center:Yes;help:Yes;resizable:No;scroll:No;status:No;');");
...
//			}
			...
			for(i=0;i<DataGridPolicy.Items.Count;i++)
			{
				...
				{
					this.RegisterStartupScript("newWindow","<script language='javascript'>alert('请在试题策略"+Convert.ToString(i+1)+"行3列中选择试题！')</script>");
					return;
				}
			}
			...
			string strTmp=ObjFun.GetValues("select PaperName from PaperInfo where PaperName='"+ObjFun.CheckString(txtPaperName.Text.Trim())+"'","PaperName");
			if (strTmp!="")
			{
			...
			else
			{
				...
				try
				{
					...
					//生成试卷
					strSql="SET NOCOUNT ON;insert into PaperInfo(PaperName,PaperType,ProduceWay,ShowModal,ExamTime,StartTime,EndTime,PaperMark,PassMark,MarkDefine,RepeatExam,FillAutoGrade,SeeResult,AutoSave,ExamAccount,ManagerAccount,TestCount,AutoJudge,CreateWay,CreateUserID,CreateDate) values ('"+strPaperName+"','"+intPaperType+"','"+intProduceWay+"','"+intShowModal+"','"+intExamTime+"','"+dtmStartTime+"','"+dtmEndTime+"','"+intPaperMark+"','"+intPassMark+"','"+intMarkDefine+"','"+intRepeatExam+"','"+intFillAutoGrade+"','"+intSeeResult+"','"+intAutoSave+"','"+intExamAccount+"','"+intManagerAccount+"','"+intTestCount+"','"+intAutoJudge+"',2,'"+intCreateUserID+"','"+dtmCreateDate+"');SELECT @@IDENTITY";
					intPaperID=ObjFun.ExecuteSql(strSql);
					if (intPaperID!=0)
					{
					...
					this.RegisterStartupScript("newWindow","<script language='javascript'>alert('新建手工组卷成功！');try{ window.opener.RefreshForm() }catch(e){};window.location='NewCustomPaper.aspx?PaperType="+Request["PaperType"]+"';</script>");
				}
				catch
				{
					ObjTran.Rollback();
					this.RegisterStartupScript("newWindow","<script language='javascript'>alert('新建手工组卷失败！');window.location='NewCustomPaper.aspx?PaperType="+Request["PaperType"]+"';</script>");
				}
				finally
				{
					SqlConn.Close();
