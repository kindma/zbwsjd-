...
namespace EasyExam.PersonalInfo
{
	...
	/// SubmExamOne 的摘要说明。
	/// </summary>
	public partial class SubmExamOne : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ((intPaperID!=0)&&(intUserScoreID!=0))
			{
				try
				{
					...
					//统计所有试题得分
					SqlCmdTest=new SqlDataAdapter("select a.RubricID,a.TestMark,b.StandardAnswer,a.UserAnswer,d.BaseTestType from UserAnswer a,RubricInfo b,PaperTestType c,TestTypeInfo d where a.RubricID=b.RubricID and b.TestTypeID=c.TestTypeID and b.TestTypeID=d.TestTypeID and a.UserScoreID="+intUserScoreID+" and c.PaperID="+intPaperID+" order by c.PaperTestTypeID,a.TestOrder",SqlConn);
					SqlDSTest=new DataSet();
					SqlCmdTest.Fill(SqlDSTest,"UserAnswer");
					dblImpScore=0;dblSubScore=0;intTestAmount=SqlDSTest.Tables["UserAnswer"].Rows.Count;
					for(i=0;i<SqlDSTest.Tables["UserAnswer"].Rows.Count;i++)
					{
						dblUserScore=0;
						strRubricID=SqlDSTest.Tables["UserAnswer"].Rows[i]["RubricID"].ToString();
						if (i==(intTestNum-1))
						{
					...
					strMessage=strMessage+"<input class='button' type='button' name='cancel'  value='结束考试' onclick='window.close()'>";
				...
				{
					//显示信息
					strMessage=strMessage+"<form id='form1' method='post' action='SubmExamOne.aspx'>";
