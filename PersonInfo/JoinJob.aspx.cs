...
namespace EasyExam.PersonalInfo
{
	...
	/// JoinJob 的摘要说明。
	/// </summary>
	public partial class JoinJob : System.Web.UI.Page
	{
		...
		private void ShowData(string strSql)
		{
			...
			{				
				...
				{
					LBStartExam.Attributes.Add("onclick","javascript:{location.replace('StartJobAll.aspx?PaperID="+DataGridPaper.Items[i].Cells[0].Text.Trim()+"&UserID="+intUserID+"&Start=yes')}");
				}
				if (DataGridPaper.Items[i].Cells[14].Text.Trim()=="逐题模式")
				{
					LBStartExam.Attributes.Add("onclick","javascript:{if (confirm('您确定要开始模拟考试/作业吗？')==true) {NewExam=window.open('StartJobOne.aspx?PaperID="+DataGridPaper.Items[i].Cells[0].Text.Trim()+"&UserID="+intUserID+"&Start=yes');}else{return false;}}");
				}
			}
