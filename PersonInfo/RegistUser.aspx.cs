	...
	public partial class RegistUser : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
                ...
                //恒恒自定义代码： 检测到用户头一次登录，就转到这个注册界面，执行自动注册
		...
		protected void ButInput_Click(object sender, System.EventArgs e)
		{
			...
			if (strTmp.Trim()!="")
			{
				this.RegisterStartupScript("newWindow","<script language='javascript'>alert('此"+txtLoginID.Text.Trim()+"帐号已经存在，请直接登录！');location.replace='/login.aspx';</script>");
				return;
			}
			...
			if (NumRowsAffected>0)
			{
				...
				{
					this.RegisterStartupScript("newWindow","<script language='javascript'>alert('帐户注册成功！');location.replace('/login.aspx?"+Request.QueryString.ToString()+"');//window.close();</script>");
				}
			}
			else
			{
