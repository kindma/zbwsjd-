...
namespace EasyExam.PersonalInfo
{
	...
	public partial class SubmJobAll : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			intTestAmount=Convert.ToInt32(Request["irow"]);//试题数量
			...
			if ((intPaperID!=0)&&(intUserScoreID!=0))
			{
				try
				{
					...
					//{
					//	Response.Write("<script>alert('作业试卷记录不存在，提交失败！')</script>");
					//	return;
					//}
					//else
					//{
							...
							case "填空类":
								...
								if (intFillAutoGrade==1)
								{
									...
									for(int j=1;j<=Math.Min(strArrUserAnswer.Length,strArrStandardAnswer.Length);j++)
									{
										...
										{
											dblUserScore=Convert.ToDouble(SqlDSTest.Tables["UserAnswer"].Rows[i]["TestMark"])*(1/Convert.ToDouble(Math.Min(strArrUserAnswer.Length,strArrStandardAnswer.Length)));
										}
									}
					...
					strMessage=strMessage+"<table border='1' bordercolorlight='#000000' bordercolordark='#FFFFFF' cellspacing='0' bgcolor='#99CCFF' width='100%'>";
					strMessage=strMessage+"<tr><td><table border='0' bgcolor='#0033CC' cellspacing='0' cellpadding='2' width='100%'>";
					strMessage=strMessage+"<tr><td style='font-size: 12pt; color: #FFFFFF' width='100%'>□作业结果</td>";
					strMessage=strMessage+"</tr></table>";
					strMessage=strMessage+"<table border='0' width='100%' cellpadding='4'>";
					strMessage=strMessage+"<tr>";
					strMessage=strMessage+"<td style='font-size: 36pt; color: black' align=center width='100%' height=300><p style='line-height: 100%'><B>"+strMsg+"</B></td></tr><tr><td colspan='2' align='center' valign='top'>";
					strMessage=strMessage+"<input class='button' type='button' name='cancel'  value='结束作业' onclick='location.replace(\"/MainLeftMenu.aspx\")'>";
					if (intSeeResult==1)
					{
						strMessage=strMessage+"<input class='button' type='button' name='cancel'  value='查看作业' onclick=javascript:{NewWin=window.open('ShowAnswer.aspx?UserScoreID="+intUserScoreID+"','ShowAnswer','titlebar=yes,menubar=no,toolbar=no,location=no,directories=no,status=yes,scrollbars=yes,resizable=no,copyhistory=yes,top=0,left=0,width=screen.availWidth,height=screen.availHeight');NewWin.moveTo(0,0);NewWin.resizeTo(screen.availWidth,screen.availHeight);}>";
				...
				catch
				{
					...
					strMessage=strMessage+"<table border='2' bordercolorlight='#000000' bordercolordark='#FFFFFF' cellspacing='0' bgcolor='#99CCFF' width='100%'>";
					strMessage=strMessage+"<tr><td><table border='0' bgcolor='#0033CC' cellspacing='0' cellpadding='2' width='100%'>";
					strMessage=strMessage+"<tr><td style='font-size:12pt; color:#FFFFFF' width='100%'>□提示信息</td>";
					strMessage=strMessage+"</tr></table>";
					strMessage=strMessage+"<table border='0' width='100%' cellpadding='4'>";
					strMessage=strMessage+"<tr><td width='100%' align='center' valign='top'><img border='0' src='../images/Information.gif'></td>";
					strMessage=strMessage+"<td style='font-size: 12pt; color: black' width='100%'>提交答卷失败，请管理员检查数据库服务器或网络是否正常，正常后再试着提交一次。</td></tr><tr><td colspan='2' align='center' valign='top'>";
