...
namespace EasyExam.NewsManag
{
	...
	/// SelectBrower 的摘要说明。
	/// </summary>
	public partial class SelectBrower : System.Web.UI.Page
	{
		...
		private void ShowSelectedData()
		{
			...
			objCmd=new SqlDataAdapter("select b.DeptID,b.DeptName from NewsUser a,DeptInfo b where a.DeptID=b.DeptID and a.NewsID="+intNewsID+" order by b.DeptName asc",objConn);
			objDS=new DataSet();
			objCmd.Fill(objDS,"DeptInfo");
			for(int i=0;i<objDS.Tables["DeptInfo"].Rows.Count;i++)
			{
			...
			objCmd=new SqlDataAdapter("select b.UserID,b.LoginID from NewsUser a,UserInfo b where a.UserID=b.UserID and a.NewsID="+intNewsID+" order by b.LoginID asc",objConn);
			objDS=new DataSet();
			objCmd.Fill(objDS,"UserInfo");
			for(int i=0;i<objDS.Tables["UserInfo"].Rows.Count;i++)
			{
		...
		protected void ButInput_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				for(i=0;i<LBSelected.Items.Count;i++)
				{
					...
					{
						ObjCmd.CommandText="insert into NewsUser(NewsID,UserID,DeptID) values("+intNewsID+",0,"+LBSelected.Items[i].Value.Replace("#","")+")";
						ObjCmd.ExecuteNonQuery();
					}
					else
					{
						ObjCmd.CommandText="insert into NewsUser(NewsID,UserID,DeptID) values("+intNewsID+","+LBSelected.Items[i].Value+",0)";
						ObjCmd.ExecuteNonQuery();
					}
				}
				ObjTran.Commit();
			...
			for(i=0;i<LBSelected.Items.Count;i++)
			{
				if (strBrower=="")
				{
					strBrower=strBrower+LBSelected.Items[i].Text;
				}
				else
				{
					strBrower=strBrower+";"+LBSelected.Items[i].Text;
			...
			this.RegisterStartupScript("newWindow","<script language='javascript'>window.returnValue='"+strBrower+"';window.close();</script>");
		}
		#endregion
